...
class DinoVisionTransformer(nn.Module):
    ...
    def _get_hook(self, facet: str):
        """
        facet: 'attn', 'query', 'key', 'value'
        """
        if facet == 'attn':
            def _hook(module, input, output):
                if not hasattr(self, '_attn_feats'):
                    self._attn_feats = []
                self._attn_feats.append(output)
            return _hook

        facet_idx_map = {'query': 0, 'key': 1, 'value': 2}
        if facet not in facet_idx_map:
            raise TypeError(f"{facet} is not supported.")
        ...
        def _inner_hook(module, input, output):
            ...
            self._attn_feats.append(qkv[facet_idx])  # [B, h, N, d_head]
        return _inner_hook

    def register_attention_hooks(self, facets=('attn',)):
        """
        facets: tuple/list of facets to hook: 'attn', 'query', 'key', 'value'
        """
        self._attn_feats = []
        self._hook_handlers = []
        for block in self.blocks:
            for facet in facets:
                # 注意: Q/K/V 需要挂在 block.attn
                if facet == 'attn':
                    handler = block.attn.attn_drop.register_forward_hook(self._get_hook(facet))
                else:
                    handler = block.attn.register_forward_hook(self._get_hook(facet))
                self._hook_handlers.append(handler)

    def remove_attention_hooks(self):
        if hasattr(self, '_hook_handlers'):
